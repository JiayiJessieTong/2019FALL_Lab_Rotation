...
seurat_workflow <- function(count_matrix, cellType_info, batch_info){
  ...
  data_seurat <- FindClusters(data_seurat, resolution = 1.5)
  data_seurat <- RunUMAP(data_seurat, dims = 1:10)
  
  n1 = length(levels(data_seurat@meta.data[["seurat_clusters"]]))
...
## check the similarity between two orders
...
check_similar(out1, out2)
check_similar(out1, out3)
check_similar(out2, out3)
